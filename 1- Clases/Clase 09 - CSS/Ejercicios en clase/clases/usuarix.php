...
//3
    ...
    public function AgregarUsuApi($request, $response){
        ...
        $datos = json_decode($array["datos"]);
        $usu = new usuarix();

        $extension = explode(".", $archivos['foto']->getClientFilename()); //OBTENGO EXTENSION
        ...
        if($usu->AgregarUsu()){
            ...
            $retorno = $response->withJson($std, 199);
        }
        return $retorno;
   }
